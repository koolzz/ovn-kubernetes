...
// NamespaceAnnotationCache stores parsed namespace annotation values keyed
// by namespace and the raw annotation string. Mirrors NodeAnnotationCache
// in shape but with a smaller set of annotations.
type NamespaceAnnotationCache struct {
	namespaces *syncmap.SyncMap[*nsAnnotationEntry]
}

// nsAnnotationEntry is the per-namespace cache entry. Each cached value
// records the raw string seen at parse time so we can short-circuit
// reparsing when the annotation hasn't changed.
...
// NewNamespaceAnnotationCache creates an empty per-namespace annotation
// parse cache.
...
// annotations. When updateCache is true, parsed values refresh the
// per-namespace cache; callers comparing an older snapshot against a newer
// one should pass false for the old snapshot so it cannot replace the
// latest cached value.
...
// DeleteNamespace removes all cached annotation parse results for the
// namespace.
