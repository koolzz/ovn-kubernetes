...
// refactor replaced the back-pointer with watchFactory.
// GetNetworkPoliciesInNamespace, which only returns core
// NetworkPolicy — silently dropping multi-net coverage. The fix walks
...
func TestReconcileNetworkPoliciesACLLoggingInNamespace_WalksSyncmap(t *testing.T) {
	...
	require.Equal(t, 3, len(bnc.networkPolicies.GetKeys()), "ReconcileNetworkPoliciesACLLoggingInNamespace must not mutate the policy syncmap")
}
