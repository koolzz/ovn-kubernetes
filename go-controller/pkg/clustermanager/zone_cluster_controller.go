...
type zoneClusterController struct {
	...
	// Transit switch IP generators allocate node transit switch port addresses.
...
func (zcc *zoneClusterController) handleAddUpdateNodeEvent(node *corev1.Node) error {
	...
	// Allocate the transit switch port IP address(es) for the node.
	...
	if err != nil {
		return fmt.Errorf("failed to marshal node %q transit switch port IP annotation, err : %w",
			node.Name, err)
	}
